...
TTabItem::TTabItem(CWnd *pParent, LPCTSTR szLabel, HICON hIcon)
{
  ...
  ASSERT(m_pCaption);
  m_pCaption->Create(szLabel, WS_CHILD|SS_CENTER|WS_VISIBLE|SS_OWNERDRAW,rect,pParent);
}
...
BEGIN_MESSAGE_MAP(TTabWnd, CWnd)
	ON_MESSAGE(WM_SIZEPARENT, &TTabWnd::OnSizeParent)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_LBUTTONUP()
	ON_WM_DESTROY()
...
void TTabWnd::createFont()
{
	...
	VERIFY(::SystemParametersInfo(SPI_GETNONCLIENTMETRICS, 0, &metrics, 0));
	// VERIFY added here, just in case...

	CWindowDC wdc(NULL);
	int nLPixY = GetDeviceCaps(wdc.m_hDC, LOGPIXELSY);
	// The 2 lines above can be deleted, I think, looks
	// like old debug code Todorovic forgot to remove
...
TTabItem *TTabWnd::addTab(CWnd *pWnd, LPCTSTR szLabel)
{
  ...
  if (m_selectedCaption == NULL)
	  m_selectedCaption = pItem->m_pCaption->GetSafeHwnd();
  return pItem;
}
...
BOOL TTabWnd::SetActivePane(int nIndex, BOOL bActivate)
{
  ...
  if (CanSetActivePane(pOldPane ? pOldPane->m_pWnd : NULL, pNewPane->m_pWnd)) {
	...
	m_selectedCaption = pNewPane->m_pCaption->GetSafeHwnd();
    // Invalidate tab
    invalidateTabArea();
...
BOOL TTabWnd::updateFrame(CFrameWnd *pFrame, CWnd *pWnd)
{
    ...
    else if (pWnd->IsKindOf(RUNTIME_CLASS(CFrameWnd)))	// 2006/10/19, YSL
    {
	CFrameWnd *pFrame = (CFrameWnd *) pWnd;
	CWnd *pView = pFrame->GetActiveView();
	if (updateFrame(pFrame, pView))
	    return TRUE;
    }
    return FALSE;
}
...
// BEGIN NEW CODE
	BOOL aDoRecalc=TRUE;
	for (int iCol=0; iCol<splitter->GetColumnCount(); iCol++)
	{
		for (int iRow=0; iRow<splitter->GetRowCount(); iRow++)
		{
...
// Handle resize
LRESULT TTabWnd::OnSizeParent(WPARAM wParam, LPARAM lParam)
{
...
//ASSERT(pClass->IsDerivedFrom(RUNTIME_CLASS(CView)));
  ASSERT(pClass->IsDerivedFrom(RUNTIME_CLASS(CWnd)));	// 2006/10/19, YSL
...
//} else if (pClass->IsDerivedFrom(RUNTIME_CLASS(CView))) {
  } else if (pClass->IsDerivedFrom(RUNTIME_CLASS(CWnd))) {  // 2006/10/19, YSL
...
BOOL TVisualFramework::Create(CWnd *pWnd)
{
  ...
  for (mapit = m_ObjectMap.begin(); mapit != m_ObjectMap.end(); mapit ++) {
    ...
    if (pObject->CanFocus() && 
	pObject->m_pWnd->IsKindOf(RUNTIME_CLASS(CView)))    // 2006/11/01, YSL
    {
...
void TTabWnd::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if (lpDrawItemStruct->CtlType == ODT_STATIC)
	{
		if (lpDrawItemStruct->itemAction == ODA_DRAWENTIRE)
		{
			...
			TCHAR szLabel[30];
			int len = ::GetWindowText(hwndItem, szLabel, sizeof(szLabel)-1);
			if (hwndItem == m_selectedCaption)
				::FillRect(hDC, &rcItem, m_BrushSel);
			::SetBkMode(hDC, TRANSPARENT);
			::TextOut(hDC, 0, 0, szLabel, len);
		}
	}
}
